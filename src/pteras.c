...
/*   Updated: 2023/11/12 23:38:36 by jcario           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	update_pteras_map(t_env *env, int new_dir, t_coords cos, int index)
{
	...
	if (new_dir == 0)
		if (!ft_strchr("1EC", env->map[cos.y + 1][cos.x]))
	...
	if (new_dir == 2)
	{
		if (!ft_strchr("1EC", env->map[cos.y][cos.x + 1]))
		{
...
void	update_pteras(t_env *env, t_coords player)
{
	...
	while (++i < env->nb_pterras)
	{
		...
		if (new_dir == 0)
			if (!ft_strchr("1EC", env->map[old_cos.y + 1][old_cos.x]))
				env->pteras[i].y++;
		if (new_dir == 1)
			if (!ft_strchr("1EC", env->map[old_cos.y - 1][old_cos.x]))
				env->pteras[i].y--;
		if (new_dir == 2)
			if (!ft_strchr("1EC", env->map[old_cos.y][old_cos.x + 1]))
				env->pteras[i].x++;
		if (new_dir == 3)
			if (!ft_strchr("1EC", env->map[old_cos.y][old_cos.x - 1]))
				env->pteras[i].x--;
		if (env->pteras[i].x == player.x && env->pteras[i].y == player.y)
			exit_game(env, 0);
